"""Plot zone-cycles/s vs resolution in 2D: new vs previous version."""
import json
import os
...
data = [json.loads(l) for l in open(os.path.join(here, "zcps_results_2d.jsonl"))]

def get(repo_new, sim, p):
    pts = sorted(
        (d["R"], d["zcps"])
        for d in data
        if d["sim"] == sim and d["p"] == p
...
fig.suptitle("GPU performance (A100, 2D hydro, RK3): optimized vs previous version", y=1.02)
fig.tight_layout()
out = os.path.join(here, "zcps_performance_2d.png")
