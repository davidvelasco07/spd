...
def quadrature_mean(
    ...
    Means are calculated with a Gauss-Legendre quadrature using ``p + 1`` points per dimension.

    Parameters
    ----------
    mesh : np.ndarray
        Physical coordinates of control-volume boundaries.
        The first axis is the spatial dimension.
        The last *d* axes define the CV boundaries along each spatial direction.
    fct : callable (pts, var) -> array
